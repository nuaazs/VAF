...
parser.add_argument('--path', type=str, default='/register/url',help='')
parser.add_argument('--file_path', type=str, default="/mnt/cti_record_data_with_phone_num",help='')
args = parser.parse_args()

result = []

spkid_paths = [os.path.join(args.file_path,_file) for _file in os.listdir(args.file_path)]
            ...
            result.append(
                {
                    ...
                    "endtime":(datetime.now()+timedelta(seconds=random.randint(60,300))).strftime("%Y-%m-%d %H:%M:%S"),
...
def register(item):
    ...
    values = {"spkid": spkid,"wav_url":wav_url,"call_begintime":begintime,"call_endtime":endtime}
    try:
