...
class CallingInterface:
    ...
    def info_neaten(self, resp, wav):
        ...
        if Args.timely_save:
            with open(Args.log_path + self.port_type + "_timely_save_file.csv", "a+") as f1:
...
def run():
    ...
    logger.info("balck test invalid data: %d.  please findv %sbalck_error_data.csv "%(len(balck_error["status"]), Args.log_path))
    ...
    logger.info("gray test invalid data: %d.  please findv %sgray_error_data.csv "%(len(gray_error["status"]), Args.log_path))
