...
def rclone_join(pattern, file_path, buckets_name):
    if pattern == "copy":
        return "rclone copy %s %s:/%s/%s"%(file_path, ARGS.rclone_name, buckets_name, file_path.split("/")[-1])
...
def run():
    ...
    while True:
        ...
        all_black_size_outdated = file_synchronization(all_black_size_outdated, dict_black_outdated, buckets_name_black, wav_path_black)
        all_gray_size_outdated = file_synchronization(all_gray_size_outdated, dict_gray_outdated, buckets_name_gray, wav_path_gray)
