...
def info_neaten(resp, total_time, wav, file_size):
    ...
    download_time = resp_json.get("download_used_time")
    vad_time = resp_json.get("vad_used_time")
    self_test_time = resp_json.get("self_test_used_time")
    classify_time = resp_json.get("classify_used_time")

    DataBase_time = resp_json.get("to_database_used_time",0)
    test_time = resp_json.get("test_used_time",0)
    GetEmbedding_time = resp_json.get("embedding_used_time",0)
...
if __name__ == "__main__":
    ...
    pb_index = ["file_name", "status", "err_msg", "inbase", "file_size",  "mean_time(s)", "download_time(ms)", 
    ...
    if args.get("IS_STORAGE") and os.path.exists(data_file):
        set_file(pb_index, args.get("FILE_NAME"))

    pattern = args.get("MODE")
    wavs = access_file(pattern)
    data = data_requests(wavs, pattern)
    res= pd.DataFrame(data, columns=pb_index)
    print(res)
