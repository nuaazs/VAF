...
def general(request_form,cfg,get_type="url",action_type="test"):
    ...
    try:
        wav_data,before_length,after_length = vad(wav_file=filepath,spkid=new_spkid,channel=wav_channel)
        wav_data = resample(wav_data=wav_data)
    except Exception as e:
        print(e)
        response = {
            "code": 2000,
    ...
    # STEP 3: Self Test
    try:
        self_test_result = encode(wav_torch=wav_data)
    except Exception as e:
        print(e)
        response = {
            "code": 2000,
