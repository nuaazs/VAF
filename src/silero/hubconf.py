...
def silero_vad(onnx=False):
    ...
    if onnx:
        model = OnnxWrapper(
            f"{hub_dir}/silero/files/silero_vad.onnx"
        )
    else:
        model = init_jit_model(
            model_path=f"{hub_dir}/silero/files/silero_vad.jit"
        )
...
def silero_lang_detector_95(onnx=False):
    ...
    with open(
        f"{hub_dir}/silero/files/lang_dict_95.json", "r"
    ) as f:
        lang_dict = json.load(f)

    with open(
        f"{hub_dir}/silero/files/lang_group_dict_95.json", "r"
    ) as f:
