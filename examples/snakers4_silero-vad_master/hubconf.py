...
def silero_vad(onnx=False):
    ...
    if onnx:
        ...
        model = init_jit_model(
            model_path=f"{hub_dir}/snakers4_silero-vad_master/files/silero_vad.jit"
        )
...
def silero_number_detector(onnx=False):
    ...
    model = Validator(url)
    utils = (get_number_ts, save_audio, read_audio, collect_chunks, drop_chunks)
...
def silero_lang_detector_95(onnx=False):
    ...
    with open(
        f"{hub_dir}/snakers4_silero-vad_master/files/lang_dict_95.json", "r"
    ) as f:
        lang_dict = json.load(f)

    with open(
        f"{hub_dir}/snakers4_silero-vad_master/files/lang_group_dict_95.json", "r"
    ) as f:
